...
module.exports = (app) => {
  ...
  fileList.forEach((file) => {
    ...
    //截取路径 app/service/custom-module/custom-service.js => custom-module/customService
    name = name.substring(
      name.lastIndexOf(`service${sep}`) + `service${sep}`.length,
      name.lastIndexOf("."),
    );

    //把'-'统一改为驼峰式，custom-moudle/custom-service.js => customMoudle.customService
